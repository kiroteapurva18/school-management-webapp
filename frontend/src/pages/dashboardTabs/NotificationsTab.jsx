...
const NotificationsTab = ({ user }) => {
  ...
  const [form, setForm] = useState({ title: "", message: "", type: user?.role === "teacher" ? "homework" : "holiday", class: "", division: "" });
  ...
  const create = async (e) => {
    ...
    setForm({ title: "", message: "", type: user?.role === "teacher" ? "homework" : "holiday", class: "", division: "" });
    fetchData();
  };
      ...
      {canCreate && (
        <form onSubmit={create} className="grid grid-cols-1 gap-2 rounded border bg-white p-4 md:grid-cols-4">
          ...
          <select className="rounded border p-2" value={form.type} onChange={(e) => setForm((p) => ({ ...p, type: e.target.value }))}>
            {user?.role === "teacher" ? <option value="homework">Homework</option> : (<><option value="holiday">Holiday</option><option value="general">General</option></>)}
          </select>
          {form.type === "homework" && (
            <>
